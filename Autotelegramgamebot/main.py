...
def show_upgrade_options(message):
    ...
    markup.add(types.InlineKeyboardButton("زمین کشاورزی", callback_data='upgrade_farm'))
    ...
    markup.add(types.InlineKeyboardButton("بانک", callback_data='upgrade_bank'))
    ...
    markup.add(types.InlineKeyboardButton("کمپ سواره‌نظام شمشیرزن", callback_data='upgrade_cavalry_swordsmen_camp'))
    markup.add(types.InlineKeyboardButton("کمپ سواره‌نظام تفنگدار", callback_data='upgrade_cavalry_gunmen_camp'))
    markup.add(types.InlineKeyboardButton("کمپ گارد ویژه", callback_data='upgrade_special_guard_camp'))
    markup.add(types.InlineKeyboardButton("کارخانه توپ متوسط", callback_data='upgrade_medium_cannon_factory'))
    markup.add(types.InlineKeyboardButton("کارخانه توپ بزرگ", callback_data='upgrade_large_cannon_factory'))
...
def show_asset_change_options(message):
    ...
    asset_types = ['stones', 'wood', 'iron', 'gold', 'money', 'food', 'meat', 'swordsmen', 'gunmen',
                   'cavalry_swordsmen', 'cavalry_gunmen', 'special_guard', 'medium_cannons', 'large_cannons', 'small_ships', 'medium_ships', 'large_ships']
...
def process_treaty_confirmation(call):
    ...
    if call.data == 'treaty_confirmed':
        ...
        new_treaties = user_treaties + "\n\n" + user_context[user_id].get('treaty_content') if user_treaties else user_context[user_id].get('treaty_content')
    ...
    if user_factories:
        ...
        cursor.execute(
            ...
            (money_collected, stones_collected, wood_collected, iron_collected, gold_collected, food_collected,
             ...
             swordsmen_collected, gunmen_collected, cavalry_swordsmen_collected, cavalry_gunmen_collected, special_guard_collected, medium_cannons_collected, large_cannons_collected,
             user_id))
        conn.commit()
